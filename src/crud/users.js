...
async function create(username, email, password){
    ...
    await status.create(user.id, 'online')
    return user
}
...
async function update(u){
    ...
    if(u.email != null && u.email !== user.email){
        user.email = u.email
    }
    if(u.tag != null && u.tag !== user.tag){
        user.tag = u.tag
    }
    if(u.profilePicture != null && u.profilePicture !== user.profilePicture){
        user.profilePicture = u.profilePicture
    }
    if(u.description != null && u.description !== user.description){
        user.description = u.description
    }

    if(u.newUsername != null && u.newUsername !== user.username){
